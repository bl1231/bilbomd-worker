...
import { IBilboMDJob, BilboMDJob } from './model/Job'
...
const initializeJob = async (job: IBilboMDJob) => {
...
const cleanupJob = async (job: BullMQJob, DBJob: IBilboMDJob) => {
  DBJob.status = 'Completed'
  DBJob.time_completed = new Date()
  await DBJob.save()
  console.log('cleanupJob', DBJob)
  // Send email to user
...
const processBilboMDJobTest = async (job: BullMQJob) => {
  const foundJob = await BilboMDJob.findOne({ _id: job.data.jobid })
    .populate({
      path: 'user',
      select: 'email'
    })
